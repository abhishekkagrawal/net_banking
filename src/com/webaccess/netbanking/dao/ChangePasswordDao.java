...
public class ChangePasswordDao {
	...
	public boolean changeLoginPassword(LoginInfoBean loginInfoBean,
			String oldLoginPassword1) {
		try {
			con = dBUtil.getconnection();
			String query = "Select * from login where userId=? and loginPassword=?";
			String password = PasswordEncrypt.encryptPassword(loginInfoBean
					.getLoginPassword());
			String oldLoginPassword = PasswordEncrypt
					.encryptPassword(oldLoginPassword1);
			...
			while (rs.next()) {
				if (password.equals(rs.getString("loginPassword"))
						|| password.equals(rs.getString("oldLoginPassword"))) {
					return false;
				} else {
	...
	public boolean changeTransactionPassword(LoginInfoBean loginInfoBean,
			String oldTransactionPassword1) {
		try {
			con = dBUtil.getconnection();
			String oldTransactionPassword = PasswordEncrypt
					.encryptPassword(oldTransactionPassword1);
			...
			rs = stmt.executeQuery();
			while (rs.next()) {
				if (password.equals(rs.getString("transactionPassword"))
						|| password.equals(rs
								.getString("oldTransactionPassword"))) {
					return false;
				} else {
