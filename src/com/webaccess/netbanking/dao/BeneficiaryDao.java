...
public class BeneficiaryDao {
	...
	public String addBeneficiary(BeneficiaryDetailBean beneficiaryDetail) {
		try {
			...
			if (rowUpdate > 0) {
				...
				String messageBody = beneficiaryDetail.getBeneficiaryName()
						+ "is added as NEFT peyee from your internet account. confirm with URN "
						+ beneficiaryDetail.getURNNumber()
						+ " . If you did not add payee, report to customer care as FRAUD.";
	...
	public ArrayList<BeneficiaryDetailBean> getDeactiveBeneficiary(
			int accountNumber, String transactionType) {

		ArrayList<BeneficiaryDetailBean> deactiveBeneficiaryList = new ArrayList<BeneficiaryDetailBean>();
		try {
			con = dBUtil.getconnection();
			...
			while (rs.next()) {
				...
				bean.setBeneficiaryAccountNumber(rs
						.getInt("beneficiaryAccountNumber"));
	...
	public ArrayList<BeneficiaryDetailBean> getActiveBeneficiary(
			int accountNumber, String transactionType) {
	...
	public boolean confirmBeneficiary(int accountNumber, int beneficiaryId,
			int URNNumber) {
		try {
			con = dBUtil.getconnection();
	...
	public String validateTransaction(TransferBean transferBean) {
		try {
			...
			con = dBUtil.getconnection();
			String pass = PasswordEncrypt.encryptPassword(transferBean
			...
			while (rs.next()) {
				if (pass.equals(rs.getString("transactionPassword"))) {
					...
					emailId = rs.getString("emailId");
					double remainingBalance = (availBalance - transferBean
					...
					if (remainingBalance > minBalance) {
						...
						if (rowUpdate > 0) {
							...
							String msg = "Hi " + name + " Account Number "
									+ transferBean.getAccountNumber()
									+ " Debited with INR "
									+ transferBean.getTransactionAmount()
									+ " on " + date1
	...
	// getting bank list
	public ArrayList<BankBean> getBankDetail() {

		ArrayList<BankBean> bankList = new ArrayList<BankBean>();
		try {
			con = dBUtil.getconnection();
