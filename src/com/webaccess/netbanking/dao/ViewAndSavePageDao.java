...
public class ViewAndSavePageDao {
	...
	public ArrayList<TransactionDetailBean> oneMonthTransactionDetail(
			int accountNumber) {
		ArrayList<TransactionDetailBean> oneMonthTransactionDeatailList = new ArrayList<TransactionDetailBean>();
		try {
			...
			while (rs.next()) {
				...
				transactionDetailBean.setTransactionId(rs
						.getInt("transactionId"));
				transactionDetailBean.setAccountNumber(rs
						.getInt("accountNumber"));
				transactionDetailBean.setCardNumber(rs.getLong("cardNumber"));
				transactionDetailBean.setChequeNumber(rs
						.getLong("chequeNumber"));
				transactionDetailBean.setDateOfCreation(rs
						.getString("dateOfcreation"));
				transactionDetailBean.setDateOfModification(rs
						.getString("dateOfModification"));
				transactionDetailBean.setTransactionAmount(rs
						.getDouble("transactionAmount"));
				transactionDetailBean.setTransactionDate(rs
						.getDate("transactionDate"));
				transactionDetailBean.setTransactionId(rs
						.getInt("transactionId"));
				transactionDetailBean.setTransactionType(rs
						.getString("transactionType"));
				transactionDetailBean.setParticulars(rs
						.getString("particulars"));
	...
	public ArrayList<TransactionDetailBean> twoMonthTransactionDetail(
			int accountNumber) {
		ArrayList<TransactionDetailBean> twoMonthTransactionDeatailList = new ArrayList<TransactionDetailBean>();
		try {
			con = dBUtil.getconnection();
	...
	public ArrayList<TransactionDetailBean> getTransactionByNumber(
			int noOftransaction, int accountNumber) {
		ArrayList<TransactionDetailBean> transactionDetailList = new ArrayList<TransactionDetailBean>();
		try {
			con = dBUtil.getconnection();
	...
	public ArrayList<TransactionDetailBean> getTransactionByDate(
			Date startDate, Date endDate, int accountNumber) {
		ArrayList<TransactionDetailBean> transactionDetailList = new ArrayList<TransactionDetailBean>();
		try {
			con = dBUtil.getconnection();
			java.sql.Date startDate1 = new java.sql.Date(startDate.getTime());
			java.sql.Date endDate1 = new java.sql.Date(endDate.getTime());
			String query = "select * from transactiondetail where  accountNumber=? and dateOfCreation between '"
					+ startDate1
					+ "' and '"
					+ endDate1
