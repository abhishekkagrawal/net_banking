...
public class BillDao {
	...
	// getting biller list
	public ArrayList<BillTypeBean> getBiller() {

		ArrayList<BillTypeBean> billerList = new ArrayList<BillTypeBean>();
		try {
			con = dBUtil.getconnection();
	...
	public String insertSBILifeInsurenceInformation(
			SBILifeInsurenceBean sBILifeInsurenceBean) {
		try {
			con = dBUtil.getconnection();
	...
	public String insertbSNLCelloneBillInformation(
			BSNLCelloneBillBean bSNLCelloneBillBean) {
		try {
			con = dBUtil.getconnection();
	...
	public ArrayList<ElectricityBillBean> getElectricityBillList(
			int accountnumber) {
		ArrayList<ElectricityBillBean> electricityBillList = new ArrayList<ElectricityBillBean>();
		try {
			con = dBUtil.getconnection();
			...
			while (rs.next()) {
				...
				electricityBillBean.setConsumerNumber(rs
						.getInt("consumerNumber"));
				electricityBillBean.setElectricityBillId(rs
						.getInt("electricityBillID"));
				electricityBillBean.setNickName(rs.getString("nickName"));
				electricityBillBean.setProcessingCycle(rs
						.getInt("processingCycle"));
				...
				electricityBillBean.setDateOfCreation(rs
						.getString("dateOfCreation"));
				electricityBillBean.setDateOfModification(rs
						.getString("dateOfModification"));
				...
				if (currMonth > paidMonth) {
					...
					electricityBillBean.setPaidStatus(rs
							.getBoolean("paidStatus"));
				}
	...
	public ArrayList<SBILifeInsurenceBean> getSBILifeInsurenceList(
			int accountnumber) {

		ArrayList<SBILifeInsurenceBean> sbiLifeInsurenceList = new ArrayList<SBILifeInsurenceBean>();
		try {
			con = dBUtil.getconnection();
			...
			while (rs.next()) {
				...
				sBILifeInsurenceBean.setDateOfCreation(rs
						.getString("dateOfCreation"));
				sBILifeInsurenceBean.setDateOfModification(rs
						.getString("dateOfModification"));
				sBILifeInsurenceBean.setDobOfpolicyHolder(rs
						.getDate("dobOfPolicyHolder"));
				sBILifeInsurenceBean.setNickName(rs.getString("nickName"));
				sBILifeInsurenceBean.setPolicynumber(rs.getInt("policyNumber"));
				sBILifeInsurenceBean.setSbiLifeinsurenceId(rs
						.getInt("sbiLifeInsurenceId"));
				...
				sBILifeInsurenceBean
						.setLastPaidDate(rs.getDate("lastPaidDate"));
				...
				if (currMonth > paidMonth) {
					...
					sBILifeInsurenceBean.setPaidStatus(rs
							.getBoolean("paidStatus"));
				}
	...
	public ArrayList<BSNLCelloneBillBean> getBSNLCelloneBillList(
			int accountnumber) {
		ArrayList<BSNLCelloneBillBean> bsnlCelloneBillList = new ArrayList<BSNLCelloneBillBean>();
		try {
			con = dBUtil.getconnection();
			...
			while (rs.next()) {
				...
				bSNLCelloneBillBean
						.setBsnlCelloneId(rs.getInt("bsnlCelloneId"));
				bSNLCelloneBillBean.setDateOfCreation(rs
						.getString("dateOfCreation"));
				bSNLCelloneBillBean.setDateOfModification(rs
						.getString("dateOfModification"));
				...
				if (currMonth > paidMonth) {
					...
					bSNLCelloneBillBean.setPaidStatus(rs
							.getBoolean("paidStatus"));
				}
	...
	public void deleteElectricityBilldetail(int electricitybillId,
			int accountNumber) {
		try {
			con = dBUtil.getconnection();
	...
	public void deleteSBILifeInsurenceDetail(int sbiLifeInsurenceId,
			int accountNumber) {
		try {
			con = dBUtil.getconnection();
	...
	// paying electric bill for particular month and sending mail to user and
	// inserting into transaction table
	public String payElectricityBill(ElectricityBillBean electricityBillBean,
			LoginInfoBean loginInfoBean, String BillRemark) {
		...
		try {
			...
			con = dBUtil.getconnection();
			String pass = PasswordEncrypt.encryptPassword(loginInfoBean
			...
			while (rs.next()) {
				if (pass.equals(rs.getString("transactionPassword"))) {
					...
					double remainingBalance = (availBalance - electricityBillBean
							.getAmount());
					if (remainingBalance > minBalance) {
						...
						if (rowUpdate > 0) {
							...
							stmt.setDate(3, date);
							stmt.setInt(4,
									electricityBillBean.getElectricityBillId());
							stmt.executeUpdate();
							String msg = "Hi " + name + " Account Number "
									+ loginInfoBean.getAccountNumber()
									+ " Debited with INR "
									+ electricityBillBean.getAmount() + " on "
									+ date1
									+ " thru Internet Banking. Aval Bal INR "
									+ remainingBalance
									+ " CR. Thank you banking with us.";
	...
	// paying BSNL Cellone bill for particular month and sending mail to user
	// and inserting into transaction table
	public String payBSNLCelloneBill(BSNLCelloneBillBean bSNLCelloneBillBean,
			LoginInfoBean loginInfoBean, String BillRemark) {
		try {
			java.util.Date date1 = new java.util.Date();
			double minBalance;
			...
			while (rs.next()) {
				if (pass.equals(rs.getString("transactionPassword"))) {
					...
					availBalance = rs.getDouble("availBalance");
					emailId = rs.getString("emailId");
					double remainingBalance = (availBalance - bSNLCelloneBillBean
							.getAmount());
					if (remainingBalance > minBalance) {
						String query1 = "insert into transactiondetail (accountNumber,transactionType,transactionDate,transactionAmount,dateOfCreation,dateOfModification,particulars) values(?,?,?,?,now(),now(),?)";
						stmt = con.prepareStatement(query1);
						...
						if (rowUpdate > 0) {
							...
							stmt.setDate(3, date);
							stmt.setInt(4,
									bSNLCelloneBillBean.getBsnlCelloneId());
							stmt.executeUpdate();

							String msg = "Hi " + name + " Account Number "
									+ loginInfoBean.getAccountNumber()
									+ " Debited with INR "
									+ bSNLCelloneBillBean.getAmount() + " on "
									+ date1
									+ " thru Internet Banking. Aval Bal INR "
									+ remainingBalance
									+ " CR. Thank you banking with us.";
	...
	// paying SBI Life Insurence bill for particular month and sending mail to
	// user and inserting into transaction table
		...
		try {
			...
			while (rs.next()) {
				if (pass.equals(rs.getString("transactionPassword"))) {
					...
					availBalance = rs.getDouble("availBalance");
					emailId = rs.getString("emailId");
					double remainingBalance = (availBalance - sBILifeInsurenceBean
							.getAmount());
					if (remainingBalance > minBalance) {
						String query1 = "insert into transactiondetail (accountNumber,transactionType,transactionDate,transactionAmount,dateOfCreation,dateOfModification,particulars) values(?,?,?,?,now(),now(),?)";
						stmt = con.prepareStatement(query1);
						...
						stmt.setString(5, "SBI Life Insurence Bill/"
								+ BillRemark);
						int rowUpdate = stmt.executeUpdate();
						if (rowUpdate > 0) {
							String query2 = "update clientdetail set availBalance=? where accountNumber=?";
							...
							stmt.setDate(2, date);
							stmt.setInt(3, sBILifeInsurenceBean
									.getSbiLifeinsurenceId());
							stmt.executeUpdate();

							String msg = "Hi " + name + " Account Number "
									+ loginInfoBean.getAccountNumber()
									+ " Debited with INR "
									+ sBILifeInsurenceBean.getAmount() + " on "
									+ date1
									+ " thru Internet Banking. Aval Bal INR "
									+ remainingBalance
									+ " CR. Thank you banking with us.";
