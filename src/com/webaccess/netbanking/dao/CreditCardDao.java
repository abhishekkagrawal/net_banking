...
public class CreditCardDao {
	...
	// getting card type
	public ArrayList<CardTypeBean> getCardType() {
		ArrayList<CardTypeBean> cardTypeList = new ArrayList<CardTypeBean>();
		try {
	...
	public String applyNewCreditCard(int accountNumber, int cardTpe,
			String nameOnCard) {
		try {
			con = dBUtil.getconnection();
			int noOfCardIssue = 0;
	...
	public ArrayList<CardDetailBean> getCreditCardetailList(int accountNumber) {
		ArrayList<CardDetailBean> creditCardDetail = new ArrayList<CardDetailBean>();
		try {
			con = dBUtil.getconnection();
			...
			while (rs.next()) {
				...
				cardDetailBean.setCreditCardNumber(rs
						.getLong("creditCardNumber"));
				cardDetailBean
						.setDateOfCreation(rs.getString("dateOfCreation"));
				cardDetailBean.setDateOfModification(rs
	...
	public String payCreditCardBill(LoginInfoBean loginInfoBean,
			CardDetailBean cardDetailBean, String BillRemark) {
		...
		try {
			...
			while (rs.next()) {
				if (pass.equals(rs.getString("transactionPassword"))) {
					...
					double remainingBalance = (availBalance - cardDetailBean
							.getUnbilledAmount());
					if (remainingBalance > minBalance) {
						...
						if (rowUpdate > 0) {
							...
							String msg = "Hi " + name + " Account Number "
									+ cardDetailBean.getAccountNumber()
									+ " Debited with INR "
									+ cardDetailBean.getUnbilledAmount()
									+ " on " + date1
