...
public class PasswordResetDao {
	...
	public String checkPasswordReset(PasswordResetBean passwordResetBean) {
		try {
			...
			if (rs != null) {
				while (rs.next()) {
					if (passwordResetBean.getMobileNumber() == rs
							.getLong("number")
							&& passwordResetBean.getOptNumber() == rs
									.getInt("optNumber")
							&& passwordResetBean.getPanCardNumber().equals(
						...
						userInfoBean.setPanCardNumber(rs
								.getString("panCardNumber"));
						...
						loginPassword = PasswordEncrypt.generatePassword();
						encryptedLoginPassword = PasswordEncrypt
								.encryptPassword(loginPassword);
						if (loginInfoBean.getLoginType().equals(
								"View And transaction")) {
							transactionPassword = PasswordEncrypt
									.generatePassword();
							encryptedTransactionPassword = PasswordEncrypt
									.encryptPassword(transactionPassword);
						}
