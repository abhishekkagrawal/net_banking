 ...
 * @author Jay Prakash
 */
public class NewTransactionNEFTAction extends ActionSupport implements
		SessionAware {
	...
	public String execute() {
		...
		if (loginInfoBean == null) {
			...
			String result = beneficiaryDelegate
					.validateTransaction(transferBean);
			if (result.equals("success")) {
				return "transfer";
			} else if (result.equals("technicalError")) {
				addActionError("Some error in transaction");
				String transactionType = "NEFT";
				activeBeneficiaryList = beneficiaryDelegate
						.getActiveBeneficiary(
								Integer.parseInt(accountNumber1[1]),
								transactionType);
				return INPUT;
