...
public class AddBillPageAction extends ActionSupport implements SessionAware {
	...
	public String addBill() {
		...
		billerList = billDelegate.getBiller();// getting biller list from
												// database
	...
	public String modifyBill() {
		UserInfoBean userInfoBean = new UserInfoBean();
		userInfoBean = (UserInfoBean) session.get("userInfo");// getting session
																// values
		if (userInfoBean == null) {
			return "error";
		} else {
	...
	public String electricity() {
		...
		if (userInfoBean == null) {
			...
			String result = billDelegate
					.insertBillInformation(electricityBillBean);
			if (result.equals("success")) {
				return "electricity";
			} else {
				billerList = billDelegate.getBiller();// getting biller list
														// from database
				stateList = billDelegate.getState();// getting state list from
													// database
	...
	public String sbiLifeInsurence() {
		...
		if (userInfoBean == null) {
			...
			String result = billDelegate
					.insertSBILifeInsurenceInformation(sBILifeInsurenceBean);
			if (result.equals("success")) {
				return "electricity";
			} else {
				billerList = billDelegate.getBiller();// getting biller list
	...
	public String bsnlCellone() {
		...
		if (userInfoBean == null) {
			...
			String result = billDelegate
					.insertbSNLCelloneBillInformation(bSNLCelloneBillBean);
			if (result.equals("success")) {
				return "electricity";
			} else {
				billerList = billDelegate.getBiller();// getting biller list
