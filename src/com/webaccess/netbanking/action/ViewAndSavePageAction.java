 ...
 * @author Jay Prakash
 */
public class ViewAndSavePageAction extends ActionSupport implements
		SessionAware {
	...
	// getting transaction detail from database by no of transaction or date
	// wise
	@Override
	public String execute() {
		...
		if (bean == null) {
			...
			if (!noOfTransaction.equals("")) {
				transactionDeatailList = viewAndSavePageDelegate
						.getTransactionByNumber(
								Integer.parseInt(noOfTransaction),
								accountNumber);
			} else {
				transactionDeatailList = viewAndSavePageDelegate
						.getTransactionByDate(startDate, endDate, accountNumber);
			}
	...
	public String oneMonthTransactionDetail() {
		...
		bean = (UserInfoBean) session.get("userInfo");
		transactionDeatailList = viewAndSavePageDelegate
				.oneMonthTransactionDetail(bean.getAccountNumber());
		session.put("transactionList", transactionDeatailList);
		return "monthTransactionDetail";
	}
	...
	public String twoMonthTransactionDetail() {
		...
		if (bean == null) {
			...
			transactionDeatailList = viewAndSavePageDelegate
					.twoMonthTransactionDetail(bean.getAccountNumber());
			session.put("transactionList", transactionDeatailList);
			return "monthTransactionDetail";
		}
	}
